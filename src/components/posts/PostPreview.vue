...
// function changeLike() {
//     if(ifLike.value) {
//         Posts.removeLike(Post.id).then(() => {
//             likes.value--;
//             ifLike.value = false;
//         }); 
//     } else {
//         Posts.addLike(Post.id).then(() => {
//             likes.value++;
//             ifLike.value = true;
//         }); 
//     }
...
<template>
    <div class="post-list" v-if="post">
        <div class="post-item">
            ...
            <div class="item-content" @click="router.push({ name: 'post', params: { postId: post.id } })">
                <div class="content-title">{{ post.title }}</div>
                <div class="content-text">{{ html2txt(post.content).slice(0, 100) }}</div>
                <el-image class="content-img" :src="post.urls[0].url" :preview-src-list="[post.urls[0].url]"
                    fit="cover" />
            </div>
                    ...
                    <div class="comment">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                ...
                                stroke="#766552" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                        {{ post.comments }}
                    </div>
                    <div class="like">
                        <mdiThumbUpOutline color="rgba(118, 101, 82, 1)" />
                        {{ post.likes  }}
                    </div>
                </div>
            </div>
